    ...
    public class DashBoardAdminRepository : IDashBoardAdminRepository
    {
        ...
        public async Task<IEnumerable<ReportSupplier>> getSupplierReportsInCurrentMonth()
        {
            ...
            .Select(s => new ReportSupplier
            {
                ...
                Status = s.Status,
                Email = s.Email,
                ActiveTours = s.tours.Count(t => t.Status == true),
                ActiveHotels = s.hotels.Count(h => h.IsVerify == true),
                ActiveRooms = s.hotels.Where(h => h.IsVerify == true).SelectMany(h => h.rooms).Count(r => r.RoomStatus == true),
                ...
                && ohh.CheckInDate.Value.Month == month)
                ...
                && ohh.Completed == true && ohh.CheckInDate.Value.Month == month)
    ...
    .OrderBy(x => x.Date)
            ...
            public async Task<IEnumerable<RevenueTourAndHotel>> getRevenueHotelByAdmin(DateTime startDate, DateTime endDate)
            {
                var Revenue = await _context.OrderHotelHeaders
                    .Where(o => o.Process == "Success" && o.Completed == true && o.CheckInDate.Value.Date >= startDate
                    && o.CheckInDate.Value.Date <= endDate)
            ...
            public async Task<IEnumerable<ReportSupplier>> getSupplierReportsInDateRange(DateTime startDate, DateTime endDate)
            {
                ...
                .Select(s => new ReportSupplier
                {
                    ...
                    .Count(oth => oth.SupplierId == s.SupplierId),
                    HotelBookings = _context.OrderHotelHeaders.Where(ohh => ohh.Process == "Success" && ohh.Completed == true
                                && ohh.CheckInDate.Value.Date >= startDate && ohh.CheckInDate.Value.Date <= endDate)
                    .Count(ohh => ohh.SupplierId == s.SupplierId),
                    TourRevenue = _context.OrderTourHeaders.Where(ohh => ohh.Process == "Success"
                    && ohh.Completed == true && ohh.TourOrderDate.Value.Date >= startDate && ohh.TourOrderDate.Value.Date <= endDate)
                        ...
                        .Sum(oth => oth.TotalPrice ?? 0),
                    HotelRevenue = _context.OrderHotelHeaders.Where(ohh => ohh.Process == "Success"
                    && ohh.Completed == true && ohh.CheckInDate.Value.Date >= startDate && ohh.CheckInDate.Value.Date <= endDate)
                        .Where(ohh => ohh.SupplierId == s.SupplierId)
                        .Sum(ohh => ohh.TotalPrice ?? 0)
                })
                .ToListAsync();
            ...
            public async Task<IEnumerable<ReportRevenueAdmin>> getRevenueAdminInMonth()
            {
                ...
                var supplierStatistics = await _context.suppliers
                    .Select(s => new
                    {
                        ...
                        s.Status,
                        s.Commission,
                        TourRevenue = _context.OrderTourHeaders.Where(oth => oth.Process == "Success" && oth.Completed == true && oth.TourOrderDate.Value.Month == month && oth.SupplierId == s.SupplierId)
                            .Sum(oth => oth.TotalPrice ?? 0),
                        HotelRevenue = _context.OrderHotelHeaders.Where(ohh => ohh.Process == "Success" && ohh.Completed == true && ohh.CheckInDate.Value.Month == month && ohh.SupplierId == s.SupplierId)
                            .Sum(ohh => ohh.TotalPrice ?? 0)
                    })
                    .ToListAsync();
                ...
                var reportRevenueAdminList = supplierStatistics
                    .Select(s => new ReportRevenueAdmin
                    {
                        ...
                        TotalRevenue = s.TourRevenue + s.HotelRevenue,
                        TotalRevenueAfterFee = (s.TourRevenue + s.HotelRevenue) * (1 - (decimal)s.Commission / 100),
                        CommissionFeeReceived = (s.TourRevenue + s.HotelRevenue) * (decimal)s.Commission / 100,
                    })
                    .ToList();
            ...
            public async Task<IEnumerable<ReportRevenueAdmin>> getRevenueAdminInDateRange(DateTime startDate, DateTime endDate)
            {
                var supplierStatistics = await _context.suppliers
                    .Select(s => new
                    {
                        ...
                        s.Status,
                        s.Commission,
                        TourRevenue = _context.OrderTourHeaders.Where(oth => oth.Process == "Success"
                        && oth.Completed == true && oth.TourOrderDate.Value.Date >= startDate
                        && oth.TourOrderDate.Value.Date <= endDate && oth.SupplierId == s.SupplierId)
                            .Sum(oth => oth.TotalPrice ?? 0),
                        HotelRevenue = _context.OrderHotelHeaders.Where(ohh => ohh.Process == "Success"
                        && ohh.Completed == true && ohh.CheckInDate.Value.Date >= startDate
                        && ohh.CheckInDate.Value.Date <= endDate
                        && ohh.SupplierId == s.SupplierId)
                            .Sum(ohh => ohh.TotalPrice ?? 0)
                    })
                    .ToListAsync();
        ...
        .Where(oh => oh.CheckInDate >= startDate && oh.CheckInDate <= endDate
        && oh.Process == "Success" && oh.Completed == true)
